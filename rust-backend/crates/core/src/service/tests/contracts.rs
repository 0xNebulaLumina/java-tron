...
// =============================================================================
// AccountPermissionUpdateContract Tests - Parity gaps from todo.md
// =============================================================================
...
fn new_test_service_with_system_enabled() -> BackendService {
    let exec_config = ExecutionConfig {
        remote: RemoteExecutionConfig {
            system_enabled: true,
            ..Default::default()
...
/// The current implementation writes permissions BEFORE checking balance (line 3715 in mod.rs).
/// In unit tests without the gRPC buffered write path, this means partial writes can persist.
/// However, in production (gRPC execution), Option A's systemic buffer approach ensures atomicity:
/// - All writes go to ExecutionWriteBuffer
/// - On Err(...), the buffer is dropped without committing
/// - See todo.md Section 2 for details
///
/// This test verifies the error message format matches Java-tron's BalanceInsufficientException.
#[test]
...
/// This uses ExecutionWriteBuffer directly to demonstrate the atomicity mechanism.
#[test]
fn test_account_permission_update_atomicity_with_write_buffer() {
    ...
    // The gRPC handler (grpc/mod.rs lines 1226-1260) implements this pattern:
    // - Creates buffer before execution
    // - Passes buffer to execution handler
    // - On Ok(_): calls buffer.commit()
    // - On Err(_): drops buffer without commit
}
...
fn test_account_permission_update_invalid_contract_type_in_operations() {
    ...
    // This should fail because bit 0 is not in AVAILABLE_CONTRACT_TYPE
