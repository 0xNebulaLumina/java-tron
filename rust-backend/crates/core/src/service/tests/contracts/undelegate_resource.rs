...
fn test_normalize_window_size_slots() {
    ...
    assert_eq!(BackendService::normalize_window_size_slots(25000, false), 25000);
    assert_eq!(BackendService::normalize_window_size_slots(28800, false), 28800);
    assert_eq!(BackendService::normalize_window_size_slots(500, true), 28800);
    assert_eq!(BackendService::normalize_window_size_slots(999, true), 28800);
    assert_eq!(BackendService::normalize_window_size_slots(28800000, true), 28800);
    assert_eq!(BackendService::normalize_window_size_slots(25000000, true), 25000);
    assert_eq!(BackendService::normalize_window_size_slots(1000, true), 1);
}

#[test]
fn test_normalize_window_size_v2_raw() {
    assert_eq!(BackendService::normalize_window_size_v2_raw(0, false), 28_800_000);
    assert_eq!(BackendService::normalize_window_size_v2_raw(0, true), 28_800_000);
    assert_eq!(BackendService::normalize_window_size_v2_raw(28_800_000, true), 28_800_000);
    assert_eq!(BackendService::normalize_window_size_v2_raw(25_000_000, true), 25_000_000);
    assert_eq!(BackendService::normalize_window_size_v2_raw(28800, false), 28_800_000);
    assert_eq!(BackendService::normalize_window_size_v2_raw(25000, false), 25_000_000);
}

#[test]
fn test_resource_increase_v2_recovery_no_usage() {
    let (new_usage, new_window, new_optimized) = BackendService::resource_increase_v2_fn(
        0, 0, 1000, 2000, 28_800_000, true,
    );
...
fn test_resource_increase_v2_recovery_with_usage() {
    ...
    let (new_usage, new_window, new_optimized) = BackendService::resource_increase_v2_fn(
        1000, 0, 10000, 10100, 28_800_000, true,
    );
...
fn test_resource_increase_v2_fully_expired() {
    ...
    let (new_usage, new_window, new_optimized) = BackendService::resource_increase_v2_fn(
        5000, 0, 1000, 30000, 28_800_000, true,
    );
    assert_eq!(new_usage, 0);
    assert_eq!(new_window, 28_800_000);
    assert!(new_optimized);
...
fn test_resource_increase_v1_recovery() {
    let (new_usage, new_window, new_optimized) = BackendService::resource_increase_v1(
        1000, 0, 10000, 10100, 28800, false,
    );
    assert_eq!(new_usage, 996);
...
fn test_resource_increase_v1_same_slot() {
    ...
    let (new_usage, new_window, _) = BackendService::resource_increase_v1(
        1000, 0, 10000, 10000, 28800, false,
    );
...
fn test_resource_increase_with_window_dispatches() {
    // V1 dispatch
    let (u1, w1, o1) = BackendService::resource_increase_with_window(
        1000, 0, 10000, 10100, 28800, false, false,
    );
    let (u1_ref, w1_ref, o1_ref) = BackendService::resource_increase_v1(
        1000, 0, 10000, 10100, 28800, false,
    );
    ...
    let (u2_ref, w2_ref, o2_ref) = BackendService::resource_increase_v2_fn(
        1000, 0, 10000, 10100, 28_800_000, true,
    );
...
fn test_un_delegate_increase_v2_basic() {
    let (new_usage, new_window, new_optimized, new_time) = BackendService::un_delegate_increase_v2_fn(
        500, 10000, 28_800_000, true,
        28_800_000, true,
        200, 10100,
    );
...
fn test_un_delegate_increase_v1_basic() {
    let (new_usage, new_window, new_optimized, new_time) = BackendService::un_delegate_increase_v1(
        500, 10000, 28800, false,
        28800, false,
        200, 10100,
    );
    assert!(new_usage > 200);
...
fn test_un_delegate_increase_dispatches() {
    ...
    let expected_v1 = BackendService::un_delegate_increase_v1(
        500, 10000, 28800, false, 28800, false, 200, 10100,
    );
...
fn test_get_all_frozen_balance_for_bandwidth() {
    ...
    use tron_backend_execution::protocol::account::{Frozen, FreezeV2};

    let mut account = Account::default();
    account.frozen.push(Frozen { frozen_balance: 1_000_000, expire_time: 0 });
    account.frozen.push(Frozen { frozen_balance: 2_000_000, expire_time: 0 });
    account.acquired_delegated_frozen_balance_for_bandwidth = 500_000;
    account.frozen_v2.push(FreezeV2 { r#type: 0, amount: 3_000_000 });
    account.frozen_v2.push(FreezeV2 { r#type: 1, amount: 999_999 }); // energy, shouldn't count
...
fn test_get_all_frozen_balance_for_energy() {
    use tron_backend_execution::protocol::Account;
    use tron_backend_execution::protocol::account::{Frozen, FreezeV2, AccountResource};

    let mut account = Account::default();
    account.account_resource = Some(AccountResource {
        frozen_balance_for_energy: Some(Frozen { frozen_balance: 1_500_000, expire_time: 0 }),
        acquired_delegated_frozen_balance_for_energy: 400_000,
        acquired_delegated_frozen_v2_balance_for_energy: 600_000,
        ..Default::default()
    });
    account.frozen_v2.push(FreezeV2 { r#type: 1, amount: 2_000_000 });
    account.frozen_v2.push(FreezeV2 { r#type: 0, amount: 888_888 }); // bandwidth, shouldn't count
...
fn test_transfer_usage_computation_bandwidth() {
    ...
    let mut transfer_usage = (receiver_net_usage as f64
        * (un_delegate_balance as f64 / all_frozen_bw as f64)) as i64;
...
fn test_transfer_usage_computation_energy() {
    ...
    let mut transfer_usage = (receiver_energy_usage as f64
        * (un_delegate_balance as f64 / all_frozen_energy as f64)) as i64;
...
fn test_resource_increase_v2_with_new_usage() {
    ...
    let (new_usage, new_window, new_optimized) = BackendService::resource_increase_v2_fn(
        0, 500, 10000, 10000, 28_800_000, true,
    );
