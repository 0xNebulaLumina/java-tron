...
fn test_market_sell_burns_fee_when_blackhole_optimization_enabled() {
    ...
    account_proto
        .asset_v2
        .insert("1000001".to_string(), 1_000_000_000);
    storage_adapter
    ...
    storage_adapter
        .put_asset_issue(b"1000001", &asset_proto, true)
        .unwrap();

    let contract_data = build_market_sell_asset_contract_data(
        owner, b"1000001", // sell token
...
fn test_market_sell_receipt_includes_order_details_on_match() {
    ...
    use crate::service::contracts::proto::{MarketOrderDetail, TransactionResultBuilder};
    ...
    let detail2 =
        MarketOrderDetail::new(vec![0x11, 0x12, 0x13], vec![0x14, 0x15, 0x16], 2000, 1000);
