...
use tron_backend_execution::{
    EngineBackedEvmStateStore, Trc10Change, TronContractType, TronExecutionContext,
...
fn test_create_contract_persists_version_1_when_allow_tvm_compatible_evm_enabled() {
    ...
    assert!(
        result.is_ok(),
        "persist_smart_contract_metadata should succeed: {:?}",
        result
    );
    ...
    assert!(
        stored_contract.is_ok(),
        "Should be able to read stored contract"
    );
    if let Ok(Some(contract)) = stored_contract {
        ...
        assert_eq!(
            contract.consume_user_resource_percent, 50,
            "consume_user_resource_percent should be preserved"
        );
        assert_eq!(
            contract.origin_energy_limit, 1000,
...
fn test_internal_create_address_derivation_formula() {
    ...
    assert_eq!(
        addr_nonce_0, addr_nonce_0_again,
...
fn test_internal_create_addresses_sequence_stable() {
    ...
    assert_ne!(
        addr1, addr2,
        "First and second CREATE addresses should differ"
    );
    assert_ne!(
        addr2, addr3,
        "Second and third CREATE addresses should differ"
    );
    assert_ne!(
        addr1, addr3,
        "First and third CREATE addresses should differ"
    );
    ...
    assert_eq!(
        addr1, addr1_check,
        "Address sequence should be reproducible"
    );
    assert_eq!(
        addr2, addr2_check,
        "Address sequence should be reproducible"
...
fn test_no_address_collisions_across_different_txids() {
    ...
    assert_ne!(
        addr1, addr2,
        "Different txids should produce different addresses"
    );
    assert_ne!(
        addr2, addr3,
        "Different txids should produce different addresses"
...
fn test_trc10_transfer_emitted_on_successful_contract_creation() {
    ...
    assert!(
        trc10_result.is_ok(),
        "TRC-10 extraction should succeed: {:?}",
        trc10_result
    );
    ...
    if let Ok(Some(trc10_change)) = trc10_result {
        match trc10_change {
            Trc10Change::AssetTransferred(transfer) => {
                assert_eq!(
                    transfer.owner_address, owner_address,
                    "Transfer should be from owner"
                );
                assert_eq!(
                    transfer.to_address, created_address,
                    "Transfer should be to created contract"
                );
                assert_eq!(
                    transfer.amount, call_token_value,
...
fn test_validation_rejects_missing_owner_account() {
    ...
    if let Err(e) = result {
        assert!(
            e.to_string().contains("no OwnerAccount") || e.to_string().contains("OwnerAccount"),
            "Error should mention missing owner account, got: {}",
            e
        );
    }
}
...
fn test_validation_rejects_contract_name_too_long() {
    ...
    let contract_data =
        build_create_smart_contract_data(&owner_address, long_name, &bytecode, 50, 1000, 0, 0, 0);
    ...
    assert!(
        error_msg.contains("32") || error_msg.contains("contractName"),
        "Error should mention name length limit, got: {}",
        error_msg
    );
}
...
fn test_validation_rejects_invalid_percent() {
    ...
    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("percent") || error_msg.contains("100"),
        "Error should mention percent bounds, got: {}",
        error_msg
    );
}
...
fn test_trc10_validation_rejects_missing_asset() {
    ...
    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("No asset"),
        "Error should be 'No asset !' for non-existent token, got: {}",
        error_msg
    );
}
...
fn test_trc10_validation_rejects_zero_asset_balance() {
    ...
    storage_adapter
        .put_asset_issue(token_id_str.as_bytes(), &asset_issue, true)
        .unwrap();
    ...
    assert!(
        result.is_err(),
        "Should fail when owner has zero token balance"
    );

    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("assetBalance must greater than 0"),
        "Error should be 'assetBalance must greater than 0.' for zero balance, got: {}",
        error_msg
    );
}
...
fn test_trc10_validation_rejects_insufficient_asset_balance() {
    ...
    use tron_backend_execution::protocol::{Account as ProtoAccount, AssetIssueContractData};
    use tron_backend_execution::ExecutionModule;
    ...
    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("assetBalance is not sufficient"),
        "Error should be 'assetBalance is not sufficient.' for insufficient balance, got: {}",
        error_msg
    );
}
