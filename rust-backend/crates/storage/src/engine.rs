...
// Define our own protobuf-compatible types for now
// These will be replaced with actual protobuf types when we integrate with the core service
...
impl StorageEngine {
    ...
    pub fn batch_write(&self, db_name: &str, operations: &[WriteOperation]) -> Result<()> {
        let db = self.get_or_init_db(db_name)?;

        let mut batch = WriteBatch::default();

        for op in operations {
            match write_operation::Type::try_from(op.r#type).unwrap_or(write_operation::Type::Put) {
                write_operation::Type::Put => {
                    batch.put(&op.key, &op.value);
                }
                write_operation::Type::Delete => {
                    batch.delete(&op.key);
                }
            }
        }
    ...
    pub fn get_stats(&self, db_name: &str) -> Result<HashMap<String, String>> {
        let db = self.get_or_init_db(db_name)?;
        ...
        stats.insert("total_keys".to_string(), total_keys);
        stats.insert("total_size".to_string(), total_size);
        stats.insert("last_modified".to_string(), chrono::Utc::now().timestamp().to_string());

        // Collect various RocksDB statistics
        if let Some(rocksdb_stats) = db.property_value("rocksdb.stats")? {
            stats.insert("rocksdb.stats".to_string(), rocksdb_stats);
        }
    ...
    pub fn health_check(&self) -> ProtoHealthStatus {
    ...
    pub fn get_property(&self, db_name: &str, property: &str) -> Result<Option<String>> {
        let db = self.get_or_init_db(db_name)?;
        Ok(db.property_value(property)?)
    }

    pub fn compact_range(&self, db_name: &str, start_key: Option<&[u8]>, end_key: Option<&[u8]>) -> Result<()> {
        let db = self.get_or_init_db(db_name)?;
