...
impl StorageModule {
    ...
    pub fn engine(&self) -> Result<&StorageEngine> {
        self.engine
            .as_ref()
            .ok_or_else(|| anyhow!("Storage engine not initialized"))
    }
}
            ...
            match engine.list_databases() {
                Ok(databases) => {
                    ...
                    if healthy_dbs == databases.len() {
                        ...
                        ModuleHealth::degraded("Some databases are not responding")
                            .with_details(details)
                    }
                }
                Err(e) => ModuleHealth::unhealthy(&format!("Cannot access databases: {}", e)),
            }
        } else {
    ...
    fn metrics(&self) -> HashMap<String, f64> {
        ...
        if let Some(engine) = &self.engine {
            if let Ok(databases) = engine.list_databases() {
                metrics.insert(
                    "storage.databases.count".to_string(),
