use revm::primitives::{Bytes, PrecompileResult, PrecompileOutput, PrecompileErrors, PrecompileError, U256};
use tracing::debug;
...
impl TronPrecompile for SystemEnergyPrice {
    fn execute(&self, input: &Bytes, gas_limit: u64) -> PrecompileResult {
        ...
        Ok(PrecompileOutput::new(GAS_COST, Bytes::from(energy_price_bytes)))
    }
}
...
impl TronPrecompile for SystemBandwidthPrice {
    fn execute(&self, input: &Bytes, gas_limit: u64) -> PrecompileResult {
        ...
        Ok(PrecompileOutput::new(GAS_COST, Bytes::from(bandwidth_price_bytes)))
    }
}
