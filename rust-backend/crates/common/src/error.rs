...
impl From<BackendError> for tonic::Status {
    fn from(err: BackendError) -> Self {
        match err {
            BackendError::Storage(msg) => {
                tonic::Status::internal(format!("Storage error: {}", msg))
            }
            BackendError::Execution(msg) => {
                tonic::Status::internal(format!("Execution error: {}", msg))
            }
            BackendError::Config(msg) => {
                tonic::Status::invalid_argument(format!("Config error: {}", msg))
            }
            BackendError::Module { module, message } => {
                tonic::Status::internal(format!("Module {} error: {}", module, message))
            }
            BackendError::Grpc(status) => status,
            BackendError::Io(e) => tonic::Status::internal(format!("I/O error: {}", e)),
            BackendError::Serialization(e) => {
