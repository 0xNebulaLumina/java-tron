...
pub fn from_tron_base58_to_bytes(base58: &str) -> Result<[u8; 21]> {
    let decoded = base58_decode(base58).map_err(|e| anyhow!("Invalid Base58 encoding: {}", e))?;
...
pub fn from_tron_address(tron_address: &str) -> Result<[u8; 20]> {
    // Decode Base58
    let decoded =
        base58_decode(tron_address).map_err(|e| anyhow!("Invalid Base58 encoding: {}", e))?;
...
mod tests {
    ...
    fn test_invalid_tron_address() {
        ...
        assert!(from_tron_address("TLsV52sRDL79HXGGm9yzwKibb6BeruhUz0").is_err());
        // Invalid checksum
    }
