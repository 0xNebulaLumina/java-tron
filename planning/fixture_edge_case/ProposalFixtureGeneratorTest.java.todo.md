...
- [ ] Skim validate logic to align fixtures with real branches:
...
- [ ] Add `validate_fail_owner_address_invalid_short`:
  - [ ] `owner_address = ByteString.copyFrom(ByteArray.fromHexString("aaaa"))` (2 bytes).
  - [ ] Expect validate error: `"Invalid address"`.
- [ ] Add `validate_fail_owner_account_not_exist`:
  - [ ] Use a valid-looking address (21 bytes, correct prefix) not present in `AccountStore`.
  - [ ] Expect validate error contains: `"Account["` + `"] not exists"`.

Parameter id/value validation (fork-independent; keep deterministic)
- [ ] Add `validate_fail_param_code_unsupported`:
  - [ ] `parameters = { 9999: 1 }`.
  - [ ] Expect validate error contains: `"Does not support code : 9999"`.
- [ ] Add `validate_fail_maintenance_interval_too_low`:
  - [ ] `parameters = { 0: 1 }` (or any value `< 3*27*1000`).
  - [ ] Expect validate error contains: `"MAINTENANCE_TIME_INTERVAL"` or `"valid range"`.
- [ ] Add `validate_fail_maintenance_interval_too_high`:
  - [ ] `parameters = { 0: 24*3600*1000 + 1 }`.
  - [ ] Expect validate error contains: `"valid range"`.
- [ ] Add `validate_fail_negative_fee_like_param`:
  - [ ] `parameters = { 2: -1 }` (e.g., `CREATE_ACCOUNT_FEE`).
  - [ ] Expect validate error contains: `"valid range is [0,"`.
- [ ] Add `validate_fail_allow_creation_of_contracts_value_zero`:
  - [ ] `parameters = { 9: 0 }`.
  - [ ] Expect validate error contains: `"ALLOW_CREATION_OF_CONTRACTS"` and `"only allowed to be 1"`.

Parameter prerequisite / dependency (fork-independent)
- [ ] Add `validate_fail_allow_tvm_transfer_trc10_prereq_not_met`:
  - [ ] Set `DynamicPropertiesStore.saveAllowSameTokenName(0)` in test setup (or per-test).
  - [ ] `parameters = { 18: 1 }`.
  - [ ] Expect validate error contains:
...
- [ ] Add `validate_fail_remove_power_gr_already_executed`:
  - [ ] Set `DynamicPropertiesStore.saveRemoveThePowerOfTheGr(-1)` before generating fixture.
  - [ ] `parameters = { 10: 1 }`.
  - [ ] Expect validate error contains: `"only allowed to be executed once"`.
- [ ] Add `validate_fail_remove_power_gr_value_not_one`:
  - [ ] Set `DynamicPropertiesStore.saveRemoveThePowerOfTheGr(0)` before generating fixture.
  - [ ] `parameters = { 10: 0 }` (or `-1`).
  - [ ] Expect validate error contains: `"REMOVE_THE_POWER_OF_THE_GR"` and `"only allowed to be 1"`.

Optional boundary-happy fixtures
- [ ] Add `happy_path_maintenance_interval_min_bound`:
  - [ ] `parameters = { 0: 3*27*1000 }`.
  - [ ] Expect `SUCCESS`.
- [ ] Add `happy_path_maintenance_interval_max_bound`:
  - [ ] `parameters = { 0: 24*3600*1000 }`.
  - [ ] Expect `SUCCESS`.
...
- [ ] Add `validate_fail_owner_address_invalid_short`:
  - [ ] `owner_address` 2 bytes (`"aaaa"`).
  - [ ] Expect validate error: `"Invalid address"`.
- [ ] Add `validate_fail_owner_account_not_exist`:
  - [ ] Use a valid-looking address not present in `AccountStore`.
  - [ ] Expect validate error contains: `"Account["` + `"] not exists"`.
- [ ] Add `validate_fail_owner_not_witness`:
  - [ ] Create an account but do not add it to `WitnessStore`.
  - [ ] Attempt approval; expect validate error contains: `"Witness["` + `"] not exists"`.

Proposal store / dynamic property inconsistency (alternate “not exists” branch)
- [ ] Add `validate_fail_proposal_missing_but_latest_num_allows_it`:
  - [ ] Set `DynamicPropertiesStore.saveLatestProposalNum(100)` and approve `proposalId=100`
        while ensuring `ProposalStore` has no entry for 100.
  - [ ] Expect validate error contains: `"Proposal[100] not exists"`.

Expiration boundary
- [ ] Add `validate_fail_expired_at_exact_boundary`:
  - [ ] Create proposal with `expirationTime == latestBlockHeaderTimestamp` (exact equality).
  - [ ] Expect validate error contains: `"expired"`.
...
- [ ] Add `happy_path_delete_without_witness_entry`:
  - [ ] Ensure owner account exists.
  - [ ] Ensure a proposal exists where `proposalAddress == ownerAddress`.
  - [ ] Delete owner from `WitnessStore` before generating fixture.
  - [ ] Expect `SUCCESS` (this pins Java behavior).

Proposal store / dynamic property inconsistency
- [ ] Add `validate_fail_proposal_missing_but_latest_num_allows_it`:
  - [ ] Set `DynamicPropertiesStore.saveLatestProposalNum(100)` and delete `proposalId=100`
        while ensuring `ProposalStore` has no entry for 100.
  - [ ] Expect validate error contains: `"Proposal[100] not exists"`.
...
- [ ] Add `happy_path_delete_even_if_already_approved_by_someone`:
  - [ ] Create proposal, add an approval from a witness, then delete by creator.
  - [ ] Expect `SUCCESS` and state becomes `CANCELED`.
