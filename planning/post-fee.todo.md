...
1) Utility: JournalPreseedUtil
- Add utility to framework for preseeding account changes in the journal using persisted post-resource state.
...
- Behavior
  ...
  - Optional: Gate to embedded mode only: `ExecutionSpiFactory.determineExecutionMode() == EMBEDDED` to avoid unnecessary work in remote or shadow.
...
2) Wire-in call sites (before VM exec)
...
- Primary path: after pre-exec resource flush
  - `framework/src/main/java/org/tron/core/db/Manager.java:1558` (after journal init), `:1559` (after setRecorder) → add `JournalPreseedUtil.tryPreseedAfterResource(trace);`
...
- Retry path: mirror the insertion
  - `framework/src/main/java/org/tron/core/db/Manager.java:1568` and `:1569` → insert same call before `:1571`.
...
4) Safety and scope
- Embedded-only effect. Remote path continues to use state changes from backend; CSV builder respects `ExecutionProgramResult` vs journal finalize.
