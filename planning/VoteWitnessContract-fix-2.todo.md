...
- [ ] Update `get_tron_power_in_sun(&self, address: &Address, new_model: bool) -> Result<u64>` to:
  - [ ] Define `const BANDWIDTH: u8 = 0; const ENERGY: u8 = 1; const TRON_POWER: u8 = 2;`
  - [ ] Initialize `total: u64 = 0`.
  - [ ] For each resource in `[BANDWIDTH, ENERGY, TRON_POWER]`:
    - [ ] Call `get_freeze_record(address, resource)`.
    - [ ] If `Some(record)`, `total = total.checked_add(record.frozen_amount).ok_or(anyhow!("Tron power overflow"))?`.
  - [ ] `Ok(total)`.
  - [ ] Logging: `info!(owner_tron, new_model, bw, energy, tron_power_legacy, total)`.
- [ ] Do not change ledger emission config.

2) Unit Tests: storage adapter tron power
- [ ] Add tests in `rust-backend/crates/execution/src/storage_adapter.rs` (existing test module):
  - [ ] `test_tron_power_bandwidth_only` → set freeze record 0 to 1_000_000; expect 1_000_000.
  - [ ] `test_tron_power_energy_only` → set freeze record 1; expect amount.
  - [ ] `test_tron_power_sum_bw_energy` → set 0+1; expect sum.
  - [ ] `test_tron_power_includes_tron_power_legacy` → set 2; expect amount.
  - [ ] `test_tron_power_all_three` → set 0+1+2; expect sum.
  - [ ] `test_tron_power_overflow_protection` → near-`u64::MAX` across two resources; expect error.

3) Integration Tests: VoteWitness with Freeze
- [ ] File: `rust-backend/crates/core/src/service.rs` tests module:
  - [ ] `vote_witness_after_freeze_v1_succeeds`
    - [ ] Create owner with sufficient balance.
    - [ ] Execute FreezeBalance (resource=BANDWIDTH, amount=1_000_000, duration>0).
    - [ ] Execute VoteWitness with votes summing to 1_000_000.
    - [ ] Expect success (no REVERT), `state_changes.len()==1`, owner AccountChange parity, energy_used=0.
  - [ ] `vote_witness_multi_freeze_accumulates`
    - [ ] Execute multiple FreezeBalance txs and then vote with total; expect success.
  - [ ] Optional: `vote_witness_energy_freeze_counts` → freeze ENERGY then vote; expect success.

4) Observability
- [ ] Ensure logs around tron power computation include:
  - [ ] Owner tron address, `new_model` flag from `support_allow_new_resource_model()`.
  - [ ] Per-resource frozen amounts and computed total.
  - [ ] Overflow errors include intermediate values.
