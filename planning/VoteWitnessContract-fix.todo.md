...
- [x] Confirm Java protobuf schema for Witness matches `protocol/src/main/protos/core/Tron.proto`:
...
- [x] Confirm Java witness writes flow in genesis init (Manager.initWitness) writes `WitnessCapsule.getData()` to `WitnessStore`.
- [x] Confirm witness DB keying: 21-byte Tron address with 0x41 prefix (see `witness_key()` implementation in Rust).

Phase 1 — Protobuf Model (Execution Crate)
- [x] Add `rust-backend/crates/execution/protos/witness.proto` containing only `protocol.Witness` mirroring Java's message.
- [x] Add `rust-backend/crates/execution/build.rs` to compile `witness.proto` via `prost-build`.
- [x] Update `rust-backend/crates/execution/Cargo.toml`:
      - Add dependency: `prost = "0.12"`.
      - Add build-dependency: `prost-build = "0.12"`.
- [x] Generate code into `OUT_DIR`; `pub mod protocol { include!(concat!(env!("OUT_DIR"), "/protocol.rs")); }` as needed.
- [x] Verify the generated Rust message matches field numbers and names; no external imports required.

Phase 2 — Deserialization Fallback in Storage Adapter
- [x] File: `rust-backend/crates/execution/src/storage_adapter.rs`.
- [x] Function: `get_witness(&self, address: &Address) -> Result<Option<WitnessInfo>>`.
- [x] Implement dual-decoder:
      1) Try protobuf decode: `protocol::Witness::decode(&*data)`.
         - [x] If OK, map fields → `WitnessInfo { address, url, vote_count }`.
             - Address mapping:
         ...
         - [x] Return `Ok(Some(witness))` if successful.
      2) Else, fall back to current legacy deserializer `WitnessInfo::deserialize(&data)`.
         - [x] If OK, return `Ok(Some(witness))`.
         - [x] If error, log error and return `Ok(None)` (unchanged semantics for corrupted data).
- [x] Logging:
...
- [x] File: `rust-backend/crates/execution/src/storage_adapter.rs`.
- [x] Function: `put_witness(&self, witness: &WitnessInfo) -> Result<()>`.
- [x] Construct `protocol::Witness`:
...
- [x] Encode with `prost::Message::encode_to_vec()` and write using existing `storage_engine.put()` with current key.
...
Phase 5 — Tests
- [x] Unit: protobuf decode success
...
- [x] Add log counters/metrics for decode path chosen (protobuf vs legacy) to monitor rollout.
- [x] Ensure no sensitive data in logs; keep addresses truncated if needed.

Phase 7 — Validation on Dataset
- [x] Rebuild backend: `cd rust-backend && cargo build --release`.
...
Edge Cases & Handling
- [x] Empty URL allowed (Java allows empty URLs on create/update in some flows).
- [x] Negative `voteCount` (should not happen in protobuf; treat as decode error → legacy fallback).
- [x] Address lengths other than 20 or 21: treat as protobuf decode error → legacy fallback.
- [x] If both decoders fail: return `Ok(None)` to preserve current semantics.
