...
- `remote.exec.apply.*` JVM flags must be removed from active runtime/script/config documentation because Java apply no longer exists.

## Implementation tasks

- [x] Replace the `RuntimeSpiImpl.execute(...)` apply branch with persisted-mirror and fail-fast ownership handling
...
- [x] Rename and retain the remote-effect detector
...
- [x] Delete the five top-level Java apply methods from `RuntimeSpiImpl`
...
- [x] Delete apply-only helper code from `RuntimeSpiImpl`
...
- [x] Clean `RuntimeSpiImpl` imports after deleting apply code
...
- [x] Restrict `RuntimeSpiImpl` creation to remote execution mode
...
- [x] Update sidecar comments so they no longer describe Java apply as active behavior
...
- [x] Keep CSV and pre-state snapshot behavior intact
  - Preserve `capturePreStateSnapshot(executionResult, context)` before persisted mirror refresh in `RuntimeSpiImpl.execute(...)`.
...
- [x] Replace `RuntimeSpiImplTest` private-apply coverage with ownership tests
...
- [x] Add persisted mirror safety tests
...
- [x] Remove active script usage of Java apply flags
...
- [x] Update Rust config comments for the new write ownership model
...
- [x] Update authoritative close-loop planning docs
...
- [x] Leave historical planning files as historical artifacts
...
- [x] Run a source search for stale B2 runtime references
...
- [x] Compile Java runtime and tests
...
- [x] Run updated `RuntimeSpiImplTest`
...
- [x] Run CSV/reporting tests that depend on sidecars
...
- [x] Run broader framework tests affected by execution SPI changes
...
- [x] Verify stale apply symbols are gone from active code paths
...
- [x] Review the final diff for accidental B3 or reporting removal
