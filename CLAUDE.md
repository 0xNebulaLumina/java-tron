...
- **Dynamic Property Storage Format Differences**: Java's DynamicPropertiesStore uses different byte sizes for different properties. CRITICAL: (1) `TOTAL_SIGN_NUM` uses `ByteArray.fromInt()` = 4 bytes, NOT 8 bytes. (2) Boolean flags like `ALLOW_MULTI_SIGN` and `ALLOW_BLACKHOLE_OPTIMIZATION` are stored as longs but must be checked with `== 1` (not `!= 0`) because Java uses `getAllowMultiSign() != 1` for disabled check. Only value 1 means enabled; values 0, 2, etc. all mean disabled.
- **String Length Parity (Java vs Rust)**: Java's `String.length()` returns UTF-16 code units, not bytes. Rust's `str::len()` returns UTF-8 bytes. For exact parity, use `s.encode_utf16().count()` in Rust. This matters for multi-byte characters: a Chinese character is 3 UTF-8 bytes but 1 UTF-16 code unit; an emoji outside BMP is 4 UTF-8 bytes but 2 UTF-16 code units (surrogate pair).