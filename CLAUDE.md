...
- **Hybrid AEXT Mode for CSV Parity**: To achieve byte-level parity between embedded and remote execution for AccountInfo AEXT (resource usage) fields, implement a "hybrid" mode where Java passes pre-execution AEXT values (bandwidth/energy windows, timestamps, usage) via gRPC, and Rust echoes these values in both old_account and new_account for unchanged fields. Key steps: (1) Add `AccountAext` and `AccountAextSnapshot` protobuf messages, (2) Extend `ExecuteTransactionRequest` with `repeated AccountAextSnapshot pre_execution_aext`, (3) In Java RemoteExecutionSPI, collect AEXT from AccountStore for owner/recipient addresses before execution, (4) In Rust, parse snapshots into HashMap<Address, AccountAext>, (5) Thread map through `convert_execution_result_to_protobuf`, (6) Add "hybrid" mode to AEXT matching logic that prefers pre-provided values over defaults. Java feature gate: `-Dremote.exec.preexec.aext.enabled=true` (default). Rust config: `accountinfo_aext_mode = "hybrid"`. This resolves state_digest_sha256 mismatches for non-VM transactions where Java's bandwidth processor updates AEXT around execution.
- **TRC-10 AssetIssueContract Implementation**: When implementing TRC-10 asset issuance in remote execution: (1) Reuse existing `trc10_enabled` flag for both TransferAsset and AssetIssue (simplifies configuration), (2) Add `get_asset_issue_fee()` method to storage adapter reading `ASSET_ISSUE_FEE` dynamic property (default: 1024 TRX = 1024000000 SUN), (3) Parse AssetIssueContract protobuf manually using varint decoding for fields: name (field 2), abbr (3), total_supply (4), precision (7), trx_num (6), num (8), start/end_time (9/10), description (20), url (21), (4) Fee handling follows blackhole optimization pattern: read `support_black_hole_optimization()` flag, if true burn fee (one AccountChange), if false credit blackhole (two AccountChanges), (5) Sort state_changes by address for CSV parity, (6) AEXT tracking uses `ResourceTracker::track_bandwidth()` with tuple destructuring `let (_path, before_aext, after_aext)`. Phase 1 focuses only on fee charging; Phase 2 will add full TRC-10 persistence via proto extension carrying semantic changes back to Java.
...
- **Signed Java Long Balance Parity**: Rust account-balance conversions must preserve Java `long` two's-complement bits in both directions. `U256 -> i64` should reinterpret the low 64 bits, and `i64 -> U256` should use `val as u64` even for negative values; rejecting negative balances breaks blackhole/system-account parity when Java legally stores high-bit balances.
- **AssetIssue Blackhole Credit Parity**: Java `adjustBalance(accountStore, blackhole, fee)` only rejects insufficient balance when the adjustment amount is negative; it does not reject a high-bit/signed-negative blackhole balance after adding a positive fee. Rust AssetIssue blackhole-credit handling must allow signed-negative post-add balances and preserve the two's-complement bits.
...
- **Rust-Persisted Multi-DB Writes**: Non-VM contracts like WitnessCreate can legitimately touch multiple databases in one transaction (e.g., `account`, `witness`, `properties`). In RR mode, don't reject `ExecutionWriteBuffer` commits just because `database_count > 1`; commit the buffer and return all touched keys so Java's B-mirror (`RuntimeSpiImpl.postExecMirror`) can refresh each affected store. Reject only actual commit/lock failures.
- **Remote Collection Timeout Artifacts**: `collect_remote_results.sh` timeout-truncated remote CSVs can look like parity row-count mismatches even when the common prefix has zero compared-field mismatches. Remote collection should wait for the configured shutdown height by default, and timeout-limited runs must be labeled incomplete instead of treated as valid parity artifacts.