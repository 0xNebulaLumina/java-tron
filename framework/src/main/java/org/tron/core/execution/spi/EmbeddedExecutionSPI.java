...
/**
 * Embedded execution implementation using the existing Java EVM.
 * This implementation wraps the current TronVM/RuntimeImpl to provide
 * the ExecutionSPI interface while maintaining backward compatibility.
 */
public class EmbeddedExecutionSPI implements ExecutionSPI {
  ...
  public CompletableFuture<ExecutionResult> executeTransaction(TransactionContext context)
    ...
    return CompletableFuture.supplyAsync(() -> {
      try {
        logger.debug("Executing transaction with embedded EVM: {}", 
  ...
  public CompletableFuture<ExecutionResult> callContract(TransactionContext context)
    ...
    return CompletableFuture.supplyAsync(() -> {
      try {
        logger.debug("Calling contract with embedded EVM: {}", 
                    context.getTrxCap().getTransactionId());
  ...
  public CompletableFuture<Long> estimateEnergy(TransactionContext context)
    ...
    return CompletableFuture.supplyAsync(() -> {
      try {
        logger.debug("Estimating energy with embedded EVM: {}", 
                    context.getTrxCap().getTransactionId());
   ...
   */
  private ExecutionResult convertProgramResultToExecutionResult(ProgramResult programResult, String runtimeError) {
    ...
    byte[] returnData = programResult.getHReturn() != null ? programResult.getHReturn() : new byte[0];
