 ...
 * This implementation provides embedded RocksDB storage within the same JVM process.
 * 
 * Pros:
 * - Low latency (no IPC overhead)
 * - Simple deployment (single process)
 * - Direct access to RocksDB features
 * 
 * Cons:
 * - No crash isolation (RocksDB issues can crash JVM)
    ...
    public CompletableFuture<HealthStatus> healthCheck() {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                boolean hasAliveDbs = databases.values().stream()
                    .anyMatch(RocksDbDataSourceImpl::isAlive);
                
                return hasAliveDbs || databases.isEmpty() ? HealthStatus.HEALTHY : HealthStatus.DEGRADED;
            } catch (Exception e) {
