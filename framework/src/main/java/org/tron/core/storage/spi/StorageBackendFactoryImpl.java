...
/**
 * Implementation of StorageBackendFactory that bridges to the full StorageSPI. This is initialized
 * by the framework module and provides storage backends to the chainbase module.
 */
@Slf4j(topic = "Storage")
public class StorageBackendFactoryImpl implements StorageBackendFactory {
  ...
  public StorageBackend createStorageBackend(StorageMode mode, String dbName) throws Exception {
    // Create the full StorageSPI using the specified mode
    StorageSPI storageSPI = StorageSpiFactory.createStorage(mode);
  ...
  /**
   * Initialize the factory and set it as the global instance. This should be called during
   * application startup.
   */
