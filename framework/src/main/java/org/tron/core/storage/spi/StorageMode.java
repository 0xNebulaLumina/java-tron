 ...
 *
 * <p>EMBEDDED - Uses embedded RocksDB via RocksDbDataSourceImpl REMOTE - Uses remote Rust storage
 * service via gRPC
 */
public enum StorageMode {
  /**
   * Embedded RocksDB storage - runs in the same JVM process. Pros: Low latency, simple deployment
   * Cons: No crash isolation, harder to scale
   */
  EMBEDDED,

  /**
   * Remote Rust storage service via gRPC - runs in separate process. Pros: Crash isolation,
   * operational flexibility, scalability Cons: Higher latency due to IPC, more complex deployment
   */
  REMOTE;
  ...
  /**
   * Get the default storage mode. Currently defaults to REMOTE for production deployments.
   *
