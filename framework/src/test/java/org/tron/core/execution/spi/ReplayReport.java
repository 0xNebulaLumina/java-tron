...
public class ReplayReport {
  ...
  public int getRequestedBlockCount() {
  ...
  public String getShadowExecutionStats() {
    return shadowExecutionStats;
  }

  public List<HistoricalReplayTool.ReplayResult> getBlockResults() {
    return blockResults;
  }
  ...
  public String getDetailedReport() {
    ...
    if (!getBlocksWithMismatches().isEmpty()) {
      ...
      for (HistoricalReplayTool.ReplayResult result : getBlocksWithMismatches()) {
        report.append(
            String.format(
                "  Block %d: %d/%d transactions, %d mismatches\n",
    ...
    report.append(
        String.format(
            "  Average transactions per block: %.1f\n",
            processedBlocks > 0 ? (double) totalTransactions / processedBlocks : 0.0));
    report.append(
        String.format(
            "  Average processing time per block: %.2f ms\n",
            processedBlocks > 0 ? (double) durationMs / processedBlocks : 0.0));
    report.append(
        String.format(
            "  Average processing time per transaction: %.2f ms\n",
  ...
  public String toCsv() {
    ...
    // Header
    csv.append(
        "block_number,total_transactions,successful_transactions,failed_transactions,mismatch_count\n");
  ...
  public double getQualityScore() {
    double completionScore =
        isComplete() ? 50.0 : (double) processedBlocks / requestedBlockCount * 50.0;
