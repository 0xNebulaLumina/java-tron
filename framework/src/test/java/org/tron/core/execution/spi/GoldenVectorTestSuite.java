 ...
 * 
 * This test suite contains curated edge-case transactions designed to thoroughly
 * test the equivalence between Java and Rust execution engines. Each test vector
 * represents a specific scenario that could potentially expose differences in
...
public class GoldenVectorTestSuite {
  ...
  // Test configuration
  private static final boolean ENABLE_SHADOW_EXECUTION = Boolean.parseBoolean(
      System.getProperty("test.shadow.enabled", "false"));
  ...
  public void setUp() {
    ...
    logger.info("Golden Vector Test Suite initialized with {} vectors (mode: {}, shadow: {})", 
               goldenVectors.size(), EXECUTION_MODE, ENABLE_SHADOW_EXECUTION);
  }
  ...
  public void tearDown() {
    if (executionSPI instanceof ShadowExecutionSPI) {
      ...
      logger.info("Shadow execution stats: {}", 
                 ((ShadowExecutionSPI) executionSPI).getMismatchStats());
    }
  }
  ...
  public void testBasicTransferVectors() throws Exception {
    List<GoldenVector> transferVectors = goldenVectors.stream()
  ...
  public void testSmartContractVectors() throws Exception {
    List<GoldenVector> contractVectors = goldenVectors.stream()
  ...
  public void testEdgeCaseVectors() throws Exception {
    List<GoldenVector> edgeCaseVectors = goldenVectors.stream()
  ...
  private void verifyGoldenVectorResult(GoldenVector vector, ExecutionSPI.ExecutionResult result) {
    ...
    Assert.assertEquals("Success mismatch for " + vector.getName(), 
                       expected.isSuccess(), result.isSuccess());
    ...
    if (expected.getEnergyUsed() > 0) {
      ...
      Assert.assertTrue(
          String.format("Energy usage mismatch for %s: expected %d, got %d (diff: %d, tolerance: %d)", 
                       vector.getName(), expected.getEnergyUsed(), result.getEnergyUsed(), 
    ...
    if (expected.getReturnData() != null) {
      Assert.assertArrayEquals("Return data mismatch for " + vector.getName(),
                              expected.getReturnData(), result.getReturnData());
    }
    
    // Verify error message for failed transactions
    if (!expected.isSuccess() && expected.getErrorMessage() != null) {
      Assert.assertTrue("Error message mismatch for " + vector.getName(),
    ...
    if (expected.getStateChangesCount() >= 0) {
      Assert.assertEquals("State changes count mismatch for " + vector.getName(),
                         expected.getStateChangesCount(), result.getStateChanges().size());
    }
  }
  ...
  private void initializeGoldenVectors() {
    ...
    logger.info("Initialized {} golden vectors across {} categories",
               goldenVectors.size(),
  ...
  private void addBasicTransferVectors() {
    // Normal transfer
    goldenVectors.add(createTransferVector(
        "basic_transfer_normal",
        OWNER_ADDRESS, RECEIVER_ADDRESS, 1000000L,
        true, 268, null, 1
    ));
    
    // Zero amount transfer
    goldenVectors.add(createTransferVector(
        "basic_transfer_zero_amount",
        OWNER_ADDRESS, RECEIVER_ADDRESS, 0L,
        true, 268, null, 1
    ));
    ...
    goldenVectors.add(createTransferVector(
        ...
        OWNER_ADDRESS, RECEIVER_ADDRESS, Long.MAX_VALUE,
        false, 0, "insufficient balance", 0
    ));
    
    // Self transfer
    goldenVectors.add(createTransferVector(
        "basic_transfer_self",
        OWNER_ADDRESS, OWNER_ADDRESS, 1000000L,
        true, 268, null, 1
    ));
  }
  ...
  private void addSmartContractVectors() {
    ...
    goldenVectors.add(createContractCallVector(
        "contract_call_simple",
        CONTRACT_ADDRESS, "getValue()", "",
        true, 1000, new byte[]{0x00, 0x00, 0x00, 0x42}, 0
    ));
    
    // Contract call with parameters
    goldenVectors.add(createContractCallVector(
        "contract_call_with_params",
        CONTRACT_ADDRESS, "setValue(uint256)", "42",
        true, 2000, new byte[0], 1
    ));
    
    // Contract call that reverts
    goldenVectors.add(createContractCallVector(
        "contract_call_revert",
        CONTRACT_ADDRESS, "revertFunction()", "",
        false, 1500, null, 0
    ));
  }
   ...
   */
  private GoldenVector createTransferVector(String name, String from, String to, long amount,
                                          boolean expectedSuccess, long expectedEnergy,
                                          String expectedError, int expectedStateChanges) {
    try {
      ...
      GoldenVector.ExpectedResult expectedResult = new GoldenVector.ExpectedResult(
   ...
   */
  private GoldenVector createContractCallVector(String name, String contractAddress,
                                              String method, String params,
                                              boolean expectedSuccess, long expectedEnergy,
                                              byte[] expectedReturnData, int expectedStateChanges) {
    try {
  ...
  private GoldenVector createExpiredTransactionVector() {
    try {
      ...
      TransactionCapsule transaction = new TransactionCapsule(
          Protocol.Transaction.newBuilder().setRawData(rawBuilder).build());

      GoldenVector.ExpectedResult expectedResult = new GoldenVector.ExpectedResult(
          false, 0, null, "TRANSACTION_EXPIRATION_ERROR", 0);

      return new GoldenVector("expired_transaction", "EDGE_CASE", transaction, false, expectedResult);
    } catch (Exception e) {
  ...
  private GoldenVector createInvalidSignatureVector() {
    try {
      ...
      GoldenVector.ExpectedResult expectedResult = new GoldenVector.ExpectedResult(
          false, 0, null, "SIGNATURE_FORMAT_ERROR", 0);
  ...
  private GoldenVector createInsufficientEnergyVector() {
    try {
      ...
      GoldenVector.ExpectedResult expectedResult = new GoldenVector.ExpectedResult(
          false, 1000, null, "OUT_OF_ENERGY", 0);

      return new GoldenVector("insufficient_energy", "EDGE_CASE", transaction, true, expectedResult);
    } catch (Exception e) {
