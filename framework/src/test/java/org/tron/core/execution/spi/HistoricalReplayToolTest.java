...
public class HistoricalReplayToolTest {
  ...
  public void testReplayBlocksWithNoData() {
    ...
    try {
      ReplayReport report =
          replayTool.replayBlocks(1000000, 5); // Use high block number that likely doesn't exist
  ...
  public void testReplayReportGeneration() {
    try {
      ...
      // Test boolean checks
      Assert.assertTrue(
          "Should be complete when no blocks processed",
          report.isComplete() || report.getProcessedBlocks() == 0);
      Assert.assertTrue(
          "Should be successful when no mismatches",
          report.isSuccessful() || report.getMismatches() == 0);
  ...
  public void testReplayWithDifferentBlockCounts() {
    ...
    for (int blockCount : blockCounts) {
      try {
        ...
        Assert.assertEquals(
            "Requested block count should match", blockCount, report.getRequestedBlockCount());
  ...
  public void testReplayResultStructure() {
    ...
    Assert.assertEquals(
        "Initial successful count should be 0", 0, result.getSuccessfulTransactions());
  ...
  public void testMismatchReportStructure() {
    ...
    HistoricalReplayTool.MismatchReport mismatch =
        new HistoricalReplayTool.MismatchReport(
    ...
    Assert.assertEquals(
        "Mismatch type should match", "ENERGY_MISMATCH", mismatch.getMismatchType());
