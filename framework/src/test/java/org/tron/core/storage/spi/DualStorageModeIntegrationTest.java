...
/**
 * Integration tests for dual storage mode functionality.
 * Tests both embedded and remote storage implementations through the factory.
 */
public class DualStorageModeIntegrationTest {
    ...
    public void testModeSwitching() throws Exception {
        ...
        Assert.assertTrue("Should create EmbeddedStorageSPI", embeddedStorage instanceof EmbeddedStorageSPI);
    ...
    private void testBatchOperations(StorageSPI storage) throws Exception {
        ...
        Map<byte[], byte[]> results = storage.batchGet(testDbName, keys).get(TIMEOUT_SECONDS, TimeUnit.SECONDS);
        ...
        Assert.assertNull("Non-existent key should return null", results.get("non-existent-key".getBytes()));
    }
