...
/**
 * Performance benchmark comparing embedded vs remote storage modes. This test helps operators
 * understand the performance characteristics of each mode.
 */
public class DualModePerformanceBenchmark extends BasePerformanceBenchmark {
  ...
  public void setUpDualMode() {
    ...
    currentMode =
        System.getProperty("test.storage.mode", "embedded"); // Default to embedded for testing
  }
  ...
  public void generateComparativePerformanceReport() throws Exception {
    ...
    System.out.printf(
        "PUT Latency  - Embedded: %.2f ms, Remote: %.2f ms (%.1fx overhead)%n",
        embeddedPutLatency, remotePutLatency, remotePutLatency / embeddedPutLatency);
    System.out.printf(
        "GET Latency  - Embedded: %.2f ms, Remote: %.2f ms (%.1fx overhead)%n",
  ...
  private void runBasicPerformanceTest() throws Exception {
    ...
    for (int i = 0; i < 100; i++) {
      storage
          .put(benchmarkDbName, ("key-" + i).getBytes(), value)
